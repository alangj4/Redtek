...
html_page = urlopen("https://m-selig.ae.illinois.edu/ads/coord_database.html")			# Open the URL
...
for link in soup.find_all('a',attrs={'href': re.compile('\.dat', re.IGNORECASE)}):				# Loop over all appropriate links on webpage
    ...
    urlretrieve(baseFlpth+link.get('href'), link.get('href').rsplit('/',1)[-1])			# Get the data from the webpage, and save it to the save data file as the link name
